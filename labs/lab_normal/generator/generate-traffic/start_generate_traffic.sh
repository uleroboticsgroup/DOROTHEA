...
##
number="$(ps -aux | grep '/usr/bin/python /generate-traffic/generate-traffic.py' | wc -l)"
if [ "$number" -ge "2" ]
then
echo "generate traffic is running"
else
echo "generate traffic stopped"
/usr/bin/python /generate-traffic/generate-traffic.py 2> /dev/null &
fi
